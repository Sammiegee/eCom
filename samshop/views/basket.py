...
from samshop.forms import BasketAddProductForm
...
class Basket(object):
    ...
    def __iter__(self):
        ...
        products = ProductDetails.objects.filter(id__in=product_ids)
...
def basket_add(request, product_id):
    ...
    return redirect('samshop:basket_detail')
...
def basket_detail(request):
    ...
    for item in basket:
        item['update_quantity_form'] = BasketAddProductForm(initial={'quantity': item['quantity'],
                                                                     'override': True})
    return render(request, 'samshop/basket_detail.html', {'basket': basket})
