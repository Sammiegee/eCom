...
def select_a_category(context):
    ...
    context.browser.find_element(By.NAME, 'clothing').click()
...
def navigate_to_any_page(context):
    base_url = urllib.request.url2pathname(context.test_case.live_server_url)
    print(base_url)
    open_url = urljoin(base_url, '/index')
    context.browser.get(open_url)
...
def enter_a_query_and_search(context):
    # use print(context.browser.page_source) to aid debugging
    # print(context.browser.page_source)
    name_textfield = context.browser.find_element(By.NAME, 'query')
    name_textfield.send_keys('shorts')
    # price_textfield = context.browser.find_element_by_name('price')
    # price_textfield.send_keys(3)
    context.browser.find_element(By.NAME, 'search_button').click()


@then("I should be able to see my search results")
def search_results(context):
    print(context.browser.page_source)
    assert 'results found' in context.browser.page_source
